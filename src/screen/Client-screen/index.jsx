...
const ClientScreen = () => {
  ...
  const handleSelectForInvoice = (client) => {
    setSelectedClientId(null);

    // Check if we're coming from NewInvoice screen with a callback
    if (route.params?.onSelectClient) {
      route.params.onSelectClient(client);
      navigation.goBack();
    } else {
      navigation.navigate('NewInvoice', { selectedClient: client });
    }
  };
      ...
      {clients?.length === 0 ? (
        <LinearGradient colors={['#55d04c39', 'rgba(76, 208, 76, 0)']} style={addClientStyles.container}>
          <View style={addClientStyles.header}>
            <TouchableOpacity onPress={() => navigation.navigate('Invoice-m')} style={addClientStyles.backButton}>
              ...
              clients.map((client) => (
                <View key={client.id} style={styles.clientWrapper}>
                  <TouchableOpacity
                    ...
                    onPress={() => handleSelectForInvoice(client)}
                  >
                    <View>
