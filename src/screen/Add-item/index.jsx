  ...
  try {
    if (editingItem) {
      ...
      await db.runAsync(
        `UPDATE items SET itemName=?, itemPrice=?, quantity=?, unit=?, discountType=?, discount=?, taxRate=?, amount=?, description=? WHERE id=?`,
        [
          updatedValues.itemName,
          updatedValues.itemPrice,
          updatedValues.quantity,
          updatedValues.unit,
          updatedValues.discountType,
          updatedValues.discount,
          updatedValues.taxRate,
      ...
      const editedItemData = {
        ...updatedValues,
        id: editingItem.id, // Preserve the original ID
      };

      // STRATEGY 1: Use callback if provided (preferred)
      if (route.params?.onItemEdit) {
        console.log('Using callback to return edited item');
        route.params.onItemEdit(editedItemData);
        navigation.goBack();
      } 
      // STRATEGY 2: Fallback to route params (keep as backup)
      else {
        console.log('Using route params to return edited item');
        navigation.navigate({
          name: "New-invoice",
          params: { editedItem: editedItemData },
          merge: true
        });
      }
    } else {
    ...
    <LinearGradient colors={['#55d04c0e', 'rgba(76, 208, 76, 0)']} style={styles.container}>
      ...
      >
        {({ handleChange, handleBlur, handleSubmit, values, errors, touched, setFieldValue }) => {
          console.log("Rendering Formik with values:", values);
          useEffect(() => {
            const amount = calculateAmount(
              values.itemPrice,
              values.quantity,
...
//         // ✅ Use callback if provided, otherwise use navigation params
//         if (route.params?.onItemEdit) {
//           route.params.onItemEdit({
//             ...values,
//             id: editingItem.id,
