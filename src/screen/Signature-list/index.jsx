...
const Signaturelist = () => {
  ...
  const onSignatureSave = route.params?.onSignatureSave;
  ...
  const handleSaveSelection = () => {
    if (!selectedId) {
      Alert.alert("Error", "Please select a signature");
      return;
    }
    ...
    if (selected) {
      if (onSignatureSave) {
        onSignatureSave(selected.fileUri);
        navigation.goBack();
      } else {
        // Fallback: navigate back with params
        navigation.navigate({
          name: 'NewInvoiceScreen',
          params: { selectedSignature: selected.fileUri },
          merge: true
        });
      }
    }
