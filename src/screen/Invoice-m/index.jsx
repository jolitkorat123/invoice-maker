...
const InvoiceScreen = () => {
  ...
  // Function to fetch invoices from database
  const fetchInvoicesFromDB = async (database) => {
    try {
      ...
      if (!tableCheck) {
        console.log("Invoices table doesn't exist yet");
        setInvoices([]);
        return;
      }
      ...
      const paymentsTableCheck = await database.getFirstAsync(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='payments'"
      );
      
      if (paymentsTableCheck) {
        // If payments table exists, use the query with payment check
        invoicesData = await database.getAllAsync(`
          SELECT 
            id,
            invoiceNo,
            clientName,
            ...
            endDate as dueDate,
            CASE 
              WHEN EXISTS (SELECT 1 FROM payments WHERE invoiceId = invoices.id) THEN 'Paid'
              ELSE 'Unpaid'
            END as status
          FROM invoices
        ...
        // If payments table doesn't exist, use a simpler query
        invoicesData = await database.getAllAsync(`
          SELECT 
            id,
            invoiceNo,
      ...
      const formattedInvoices = invoicesData.map(invoice => ({
        ...
        message: invoice.status === 'Paid' ? t('paid') : t('unpaid'),
        dbId: invoice.id // Store the database ID for reference
      }));
  ...
  useFocusEffect(
    useCallback(() => {
      if (db) {
        loadItems(db);
        fetchInvoicesFromDB(db); // Fetch invoices when screen is focused
      }
    }, [db])
  );
  ...
  useEffect(() => {
    const initDb = async () => {
      try {
        ...
        setDb(database);
        await loadItems(database);
        await fetchInvoicesFromDB(database); // Fetch invoices after DB initialization
      } catch (err) {
        console.error('DB Init Error:', err);
      }
    };
  ...
  const totalSales = invoices.reduce((sum, invoice) => sum + (invoice.amount || 0), 0);
  const totalReceived = invoices
    .filter(invoice => invoice.status === 'Paid')
  ...
  const handleInvoiceStatusChange = async (newStatus) => {
    ...
    try {
      ...
      // Use the database ID stored in the invoice object
      const invoiceId = selectedInvoice.dbId;
      ...
      // If changing to Paid, add a payment record
      if (newStatus === 'Paid') {
        await db.runAsync(
          'INSERT INTO payments (invoiceId, amount, paymentDate, paymentMethod) VALUES (?, ?, ?, ?)',
          [invoiceId, selectedInvoice.amount || 0, new Date().toISOString(), 'Cash']
        );
      } 
      // If changing from Paid to Unpaid, remove payment record
      else if (selectedInvoice.status === 'Paid' && newStatus === 'Unpaid') {
        await db.runAsync('DELETE FROM payments WHERE invoiceId = ?', [invoiceId]);
      }
      
      // Refresh the invoices list
      await fetchInvoicesFromDB(db);
      setShowInvoiceStatusModal(false);
    } catch (error) {
